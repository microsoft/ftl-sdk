...
ftl_status_t media_init(ftl_stream_configuration_private_t *ftl) {
  ...
  do {
    ...
    // The state needs to be set before the thread actually begins to avoid race condition inside
    // _internal_media_destroy which can potentially skip os_destroy_thread(media->send_thread).
    ...
    // The state needs to be set before the thread actually begins to avoid race condition inside
    // _internal_media_destroy which can potentially skip os_destroy_thread(media->ping_thread).
